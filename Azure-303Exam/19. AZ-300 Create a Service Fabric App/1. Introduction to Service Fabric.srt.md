...
have expected.
Right so some bug in some far off way got triggered and you can't even justify how they're related but
you're triggering it because you're always deploying things that don't need to be deployed.
So smaller teams are are better in this case.
Now when you look at Asa's implementation of micro services it's called Azure service fabric service
fabric runs as a pool of servers or a cluster and all services can be deployed to various services servers
as they're needed.
Now there's a there's a dense deployment model so you're not just having one program on one server you
could have tens or hundreds of services deployed to each server.
